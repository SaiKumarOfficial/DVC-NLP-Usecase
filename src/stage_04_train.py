...
STAGE = "Stage 04 Train" ## <<< change stage name 
...
def main(config_path, params_path):
    ...
    featurized_train_data_path = os.path.join(featurized_data_dir, artifacts["FEATURIZED_TRAIN_DATA"])
    ...
    model = RandomForestClassifier(
        n_estimators=n_estimators,min_samples_split = min_split, n_jobs = 2, random_state= seed
        )
    model.fit(X,labels)

    joblib.dump(model,model_path)
    logging.info(f"Model is trained and save at:{model_path}")
